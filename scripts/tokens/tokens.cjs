...
const fileStringCSS = `
/* 
 * This file is automatically generated by scripts/tokens.cjs! 
 */
...
const theme = {
  color: {
${drawJSPropLines(processed.color.semantic[SEMANTIC_MODES.light], "    ")}
  },
  size: {
${drawJSPropLines(processed.size.semantic.default, "    ")}
  }
}
...
function drawJSPropLines(lines, indent = "  ") {
  return lines
    .sort((a, b) => (a.property > b.property ? 1 : -1))
    .map((l) => `${indent}${l.propertyName}: "var(${l.property})"`)
    .join(",\n");
}
...
fs.writeFileSync("./src/theme.css", fileStringCSS);
fs.writeFileSync("./src/theme.ts", fileStringTS);
